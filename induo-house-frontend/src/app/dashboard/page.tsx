...
import {
  Building2, User, Mail, Phone, Shield,
  Plus, ArrowRight, TrendingUp, Heart
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const loadFavs = async () => {
      try {
        ...
        if (ids.length === 0) { setFavLoading(false); return; }
        const data = await getProperties({ size: 50 });
        const matched = data.content.filter(p => ids.includes(p.id));
            ...
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-card)', borderRadius: 20, padding: 28, marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {[
                  ...
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: idx < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
            ...
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-card)', borderRadius: 20, padding: 28 }}>
              ...
              {favLoading ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 16 }}>
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid var(--border-card)', background: 'var(--bg-card)' }}>
                ...
                <div style={{ textAlign: 'center', padding: '40px 20px', borderRadius: 14, border: '1px dashed var(--border)', background: 'var(--bg-card)' }}>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-faint)', margin: 0 }}>Kliknij ❤️ na ogłoszeniu żeby dodać do ulubionych</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 16 }}>
                  {favProperties.map((p, i) => (
                    <div key={p.id} style={{ animation: `fadeUp 0.4s cubic-bezier(0.22,1,0.36,1) ${i * 0.06}s both` }}>
